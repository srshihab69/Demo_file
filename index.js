...
const strings = {
    ...
    help: 
        ...
        ` · 📷 Send Photo/Video → Get file_id & Share Direct Link\n` +
        ...
        if (text === '/start') {
            ...
            if (mediaData) {
                ...
                if (mediaData.fileType === 'photo' && mediaData.fileId) {
                    ...
                    // Fallback to forwarding or redirect info if fileId isn't stored directly
                    await bot.sendMessage(chatId, headerText + `🔗 <b>Direct Link:</b>\n<code>${trimmedLink}</code>`, {
                        parse_mode: 'HTML',
                        reply_markup: { inline_keyboard: [[{ text: '📥 Open Media', url: trimmedLink }]] }
                    });
                    return;
                }
            }
            ...
            if (msg.photo || msg.video || msg.animation || msg.document || msg.audio || msg.voice) {
                ...
                if (fileObj && fileObj.file_id) {
                    ...
                    try {
                        ...
                        if (fileInfo && fileInfo.file_path) {
                            ...
                            // Save fileId and user information in mediaStore
                ...
                if (customDirectLink && customDirectLink !== 'N/A') {
                    // Replaced download button with share button (switch_inline_query shares only the clean direct link)
