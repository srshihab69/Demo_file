...
async function uploadToCatbox(fileBuffer, fileName) {
    try {
        ...
        return response.data; // Returns direct permanent URL
    } catch (error) {
...
async function uploadToLitterbox(fileBuffer, fileName, time) {
    try {
        ...
        return response.data; // Returns direct timed URL
    } catch (error) {
...
const strings = {
    welcome: (name) => 
        ...
        `<blockquote>Welcome to <b>TG Meta69 Bot!</b> Explore user & media information, manage media tools, upload to Cloud storage with custom expiration options, and download TikTok videos with ease. 🚀</blockquote>`,
...
app.post(`/api/webhook`, async (req, res) => {
    try {
            ...
            if (data.startsWith('upload_')) {
                ...
                try {
                    ...
                    if (uploadType === 'catbox') {
                        ...
                        // Map time values: 1 -> 1h, 24 -> 24h, 48 -> 72h (Litterbox preset), 72 -> 72h
                        let litterTime = '1h';
                        if (timeVal === '24') litterTime = '24h';
                        else if (timeVal === '48') litterTime = '72h'; // maps 48h to 72h preset safely
                    ...
                    // Store direct URL mapping for srmeta payload
                    ...
                    const successText = `> ✅ <b>File Uploaded Successfully!</b>\n\n` +
                        `<blockquote>✨ <b>Cloud Direct Link:</b>\n` +
                        `🔗 <code>${directUrl}</code>\n\n` +
                        `🌐 <b>Viewer Link:</b>\n` +
                        `🔗 <code>${browserDirectLink}</code></blockquote>`;
        ...
        // Handle /start with deep link payload (e.g., /start srmeta_xxxx or /start sr69)
            ...
            if (parts.length > 1) {
                ...
                if (param.startsWith('srmeta_') || param === 'sr69') {
                    if (mediaStore.has(param)) {
                        ...
                        // If triggered via sr69 generically without unique mapping, show instructions or recent media
                        await bot.sendMessage(chatId, `<blockquote>✨ <b>TG Meta69 Bot Media Hub</b></blockquote>\n\n<blockquote>Welcome via bot share link! Send any photo, video, or document to generate cloud links.</blockquote>`, { parse_mode: 'HTML' });
                        return;
                    }
                }
            }
        ...
        // ==============================================================================
            ...
            // ================= FORWARD MESSAGE FILTER =================
            // As requested: forward messages containing pic/video/file will NOT show direct links or inline buttons.
            ...
            if (fileObj && fileObj.file_id) {
                ...
                if (!isForwarded) {
                    // Store pending upload reference so user can choose cloud expiration buttons
                    ...
                    // 5 Inline Buttons: 4 Litterbox options (1, 24, 48, 72) + 1 Catbox option (Always active permanent)
                    inlineButtons.push([
                    ...
                    // If forwarded, show file info without cloud links or inline upload buttons as specified
                    finalMessage += `> ✨ <b>${mType}</b>\n\n` +
                        `> 🆔 File ID: <code>${mId}</code>${mExtra}`;
                }
            }
